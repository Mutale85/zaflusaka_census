<?php
	...
	if (!empty($personnel_id)) {
		$update = $connect->prepare("UPDATE service_personnel SET `rank` = ?, `firstname` = ?, `surname` = ?, `trade_branch` = ?, `phone_number` = ?, `unit` = ?, `marital_status` = ?, `gender` = ?, `spouse_firstname` = ?, `spouse_surname` = ?, `spouse_service_number` = ?, `spouse_rank` = ?, `spouse_nrc` = ?, `spouse_phone_number` = ?, `spouse_occupation` = ?, `spouse_employer` = ?, `quarter_number` = ? WHERE id = ? AND parent_id = ? ");
		$ex = $update->execute(array( $rank, $firstname, $surname, $trade_branch, $phone_number, $unit, $marital_status, $gender, $spouse_firstname, $spouse_surname, $spouse_service_number, $spouse_rank, $spouse_nrc, $spouse_phone_number, $spouse_occupation, $spouse_employer, $quarter_number, $personnel_id, $parent_id));
		if($ex){
		...
		if($ex){
			...
			setcookie("service_number", base64_encode($personnel_id), time()+60*60*24*7, '/');
		}else{
			exit();
		}	
	}